...
def _build_timeline_html(source_df):
    """Ιστορικό ασφάλισης ανά Ταμείο - Τύπο Ασφάλισης με οπτικές μπάρες.
...
def _totals_raw_records_for_js(raw_df, month_days=25, year_days=300):
    """Λίστα ωμών εγγραφών για client-side φιλτράρισμα ημερομηνιών (Lite). Περιλαμβάνει g/c για αποδοχές/εισφορές."""
    ...
    # Αποδοχές/εισφορές (αριθμητικά για άθροισμα στο JS)
    def _gross(r):
...
def _build_totals_with_filters(display_summary, raw_df=None, desc_map=None, warning_types=None):
    ...
    def _dropdown_checkboxes(name, options, data_attr):
        ...
        return f'''<div class="filter-group filter-dropdown" data-filter-name="{html_mod.escape(name)}" data-filter-key="{html_mod.escape(data_attr)}">
  <div class="filter-dropdown-trigger" tabindex="0" role="button" aria-haspopup="listbox" aria-expanded="false" title="Επιλέξτε {html_mod.escape(name)}">{html_mod.escape(name)}</div>
  <div class="filter-dropdown-panel" role="listbox">
    <div class="filter-options">{items}</div>
  </div>
  <div class="filter-selected-label" aria-live="polite"></div>
</div>'''
    ...
    (function(){
      ...
      function applyTotalsFilters() {
        ...
        var paketoChecked = [];
        if (section) section.querySelectorAll('.totals-filters input[name="Πακέτο"]:checked').forEach(function(cb){ paketoChecked.push(cb.value); });
        var tameioChecked = [];
        if (section) section.querySelectorAll('.totals-filters input[name="Ταμείο"]:checked').forEach(function(cb){ tameioChecked.push(cb.value); });
        var sel = { paketo: paketoChecked, tameio: tameioChecked };
        if (section) {
          section.querySelectorAll('.totals-filters .filter-dropdown').forEach(function(dd) {
            ...
            if (label) label.textContent = vals.length ? vals.join(', ') : '';
          });
        }
        ...
        if (Array.isArray(_totalsRawRecords) && _totalsRawRecords.length > 0) {
          ...
          filtered.forEach(function(rec) {
            var key = rec.p + '|' + rec.t;
            if (!groups[key]) groups[key] = { p: rec.p, t: rec.t, apo: rec.apo, eos: rec.eos, h: 0, gross: 0, contrib: 0, n: 0 };
            var g = groups[key];
            g.h += rec.h;
            g.gross += (rec.g !== undefined && !isNaN(rec.g)) ? rec.g : 0;
            g.contrib += (rec.c !== undefined && !isNaN(rec.c)) ? rec.c : 0;
            g.n += 1;
      ...
      document.querySelectorAll('#totals-section .totals-filters input').forEach(function(inp) {
        inp.addEventListener('change', applyTotalsFilters);
      });
      document.querySelectorAll('#totals-section .totals-filters input.filter-date').forEach(function(inp) {
        inp.addEventListener('input', applyTotalsFilters);
        inp.addEventListener('change', applyTotalsFilters);
      });
      document.querySelectorAll('#totals-section .totals-filters .filter-dropdown-trigger').forEach(function(tr) {
      ...
      document.addEventListener('click', function() {
        var section = document.getElementById('totals-section');
        if (section) section.querySelectorAll('.totals-filters .filter-dropdown.open').forEach(function(dd) {
          dd.classList.remove('open');
          var t = dd.querySelector('.filter-dropdown-trigger');
          if (t) t.setAttribute('aria-expanded', 'false');
        });
      });
      document.querySelectorAll('#totals-section .totals-filters .filter-dropdown-panel').forEach(function(panel) {
    ...
    section_html = f"""
    <section class="print-section" id="totals-section">
...
table.print-table thead th {
  ...
  text-align: left; font-weight: 400; font-size: 9px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}
...
<div style="margin-top:12px;font-size:9px;color:#888;text-align:left;">© Syntaksi Pro - my advisor</div>
    ...
    # Escape print HTML for embedding in JS (για popup εκτύπωσης)
    print_js = json.dumps(print_html).replace("</script>", "<\\/script>")
    dl_safe = re.sub(r'[<>:"/\\|?*]', '', (client_value or "Αναφορά").strip())[:60].strip() or "Αναφορά"

    # --- Interactive viewer: ενιαίο builder (ίδιο με κύρια, μόνο τίτλος/όνομα διαφορετικά) ---
    tab_entries_with_note = [(tid, label, (LITE_EXCLUSION_NOTE if tid == "count" else "") + content) for (tid, label, content) in tab_entries]
    download_filename = f"{dl_safe} - ATLAS Lite.html"
    html_doc = build_viewer_html_document(
        tab_entries_with_note,
        client_name=client_name or "",
        print_html=print_html,
        download_filename=download_filename,
        app_title="ATLAS Lite",
        app_subtitle="Προεργασία φακέλου",
        print_brand_suffix="ATLAS Lite",
        add_exclusion_note_for_count=False,
        print_styles=print_styles_content,
    )
